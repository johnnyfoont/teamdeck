...
(function () {
  ...
  const isoLocalDate = (value) => { const d=value instanceof Date?value:new Date(value); return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`; };
  ...
  const storedHiringMeetings = read(hiringKey, null);
  if (storedHiringMeetings && typeof hiringMeetings !== 'undefined' && Array.isArray(hiringMeetings)) { hiringMeetings.splice(0, hiringMeetings.length, ...storedHiringMeetings); }
  ...
  window.deleteHiringEvent = (id) => { const index=hiringMeetings.findIndex(x=>Number(x.id)===Number(id)); if(index<0)return; hiringMeetings.splice(index,1); saveMeetings(); closeModal(); renderHiringCalendar(); renderUpcomingEvents(); toast('Событие удалено'); };
  ...
  function upcoming() { const now=new Date(), startDay=today(), endDay=isoLocalDate(new Date(now.getFullYear(),now.getMonth(),now.getDate()+7)), out=[]; const inRange=(date)=>String(date||'')>=startDay&&String(date||'')<=endDay; hiringMeetings.forEach(x=>{if(inRange(x.date))out.push({date:x.date,time:x.time,title:x.title||x.stage,sub:x.candidate||'Календарь найма',color:colorFor(x),kind:'hiring'});});offEvents().forEach(x=>{if(inRange(x.date))out.push({date:x.date,time:x.time,title:x.title,sub:'Календарь оффбординга',color:x.color,kind:'off'});});(typeof offboardingPeople!=='undefined'?offboardingPeople:[]).forEach(p=>{if(p.lastDay){if(inRange(p.lastDay))out.push({date:p.lastDay,time:'09:00',title:'Последний рабочий день',sub:p.name,color:'#f59e0b',kind:'off'});}});
    const onboardingDone=read('teamdeck-onboarding-checks',{}), offboardingDone=read('teamdeck-offboarding-checks',{});
    if(typeof onboardingPeople!=='undefined'&&typeof onboardingChecks!=='undefined') onboardingPeople.forEach(p=>{const tasks=onboardingChecks[Math.max(0,Number(p.stage)||0)]||[],done=onboardingDone[p.id]||[],task=tasks.find((_,i)=>!done.includes(i));if(task)out.push({date:today(),time:'',title:task,sub:`${p.name} · Онбординг`,color:'#6b7280',kind:'check'});});
    if(typeof offboardingPeople!=='undefined'&&typeof offboardingChecks!=='undefined') offboardingPeople.forEach(p=>{const tasks=offboardingChecks[Math.max(0,Number(p.stage)||0)]||[],done=offboardingDone[p.id]||[],task=tasks.find((_,i)=>!done.includes(i));if(task)out.push({date:today(),time:'',title:task,sub:`${p.name} · Оффбординг`,color:'#6b7280',kind:'check'});});
    const sortEvents=(a,b)=>{ const day=String(a.date||'').localeCompare(String(b.date||'')); if(day)return day; return String(a.time||'23:59').localeCompare(String(b.time||'23:59')); };
    return out.sort(sortEvents);}
  window.renderUpcomingEvents=()=>{const box=document.getElementById('upcomingEventsList');if(!box)return;const a=upcoming();box.innerHTML=a.length?a.map(x=>`<button class="upcoming-event ${x.kind==='check'?'checklist-event':''}" style="--event-color:${esc(x.color)}">${x.kind==='check'?'':`<span class="upcoming-event-date">${dateLabel(x.date)}${x.time?` · ${esc(x.time)}`:''}</span>`}<span><b>${esc(x.title)}</b><small>${esc(x.sub)}</small></span></button>`).join(''):'<div class="empty">На ближайшую неделю событий нет</div>';};
  ...
  window.addEventListener('storage', (event) => { if (event.key===hiringKey || event.key===offKey) { if (event.key===hiringKey && typeof hiringMeetings!=='undefined') { try { const next=JSON.parse(event.newValue||'[]'); if(Array.isArray(next)) hiringMeetings.splice(0,hiringMeetings.length,...next); } catch (_) {} } if(typeof renderHiringCalendar==='function')renderHiringCalendar(); if(typeof renderOffboardingCalendar==='function')renderOffboardingCalendar(); renderUpcomingEvents(); } });
