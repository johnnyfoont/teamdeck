(function () {
  ...
  function clientDeviceDetail() {
    const ua = navigator.userAgent || '';
    if (clientDeviceCategory() === 'Планшет') return /safari/i.test(ua) ? 'iPad · Safari' : 'iPad';
    if (clientDeviceCategory() === 'Смартфон') return /android/i.test(ua) ? 'Android' : 'iPhone';
    return /macintosh/i.test(ua) ? 'Текущий браузер · macOS' : 'Текущий браузер';
  }
  function state() {
    try {
      const stored = JSON.parse(localStorage.getItem(STORAGE_KEY) || '{}');
      const value = { ...seed, ...stored, sessions: stored.sessions || seed.sessions, events: stored.events || seed.events, blocked: stored.blocked || seed.blocked, blockedMeta: stored.blockedMeta || seed.blockedMeta };
      const current = value.sessions.find(item => item.id === 'demo-current' || (item.identity === 'demo' && item.current));
      if (current) { current.device = clientDeviceCategory(); current.deviceDetail = clientDeviceDetail(); }
      return value;
    } catch (_) { return { ...seed }; }
  }
  ...
  function sessionIcon(method) { const text = String(method || '').toLowerCase(); if (/telegram|телеграм/.test(text)) return '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m4 11 15-6-4 14-4-5-4-3Z"/><path d="m11 14 4-5"/></svg>'; if (/одноразовый|otp|код/.test(text)) return '<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="3.5" y="5" width="17" height="14" rx="2"/><path d="m4 7 8 6 8-6"/></svg>'; return '<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="8.5" cy="15.5" r="3"/><path d="m10.5 13.5 8-8 2 2-2 2-1.5-1.5-2 2 1.5 1.5-2 2"/></svg>'; }
  ...
  function eventCountry(item) { const raw = `${item.country || ''} ${item.location || ''} ${item.meta || ''}`; const code = raw.match(/(?:^|[·\s])([A-Z]{2})(?=$|[·\s])/i)?.[1]?.toUpperCase(); if (code && countryCatalog[code]) return { name: countryCatalog[code][0], flag: countryCatalog[code][1] }; const found = Object.values(countryCatalog).find(value => raw.includes(value[0])); return found ? { name: found[0], flag: found[1] } : null; }
  function eventMetaLabel(item) { const base = String(item.meta || item.location || '—').replace(/\s*[·•]\s*(?:[A-Z]{2}|Россия|Словакия|Германия|Франция|Великобритания|Китай|Сингапур|Япония|США|Канада|Австралия|Нидерланды)\s*$/i, '').trim(); const country = eventCountry(item); return country ? `${base} · ${country.flag} ${country.name}` : base; }
  function filteredSessions(s) {
    const query = modalQuery.toLowerCase();
    const items = s.sessions.filter(item => !isCurrentSession(item) && !query || [item.user, item.identity, item.method, item.device, item.location].some(value => String(value || '').toLowerCase().includes(query)))
  ...
  function render() {
    ...
    const s = state(); const active = s.sessions.filter(x => x.status === 'active' && !x.current); const previewSessions = s.sessions.filter(x => !isCurrentSession(x)).slice(0, 12); const previewEvents = s.events.slice(0, 12);
    root.innerHTML = `<div class="security-page"><div class="top"><div class="title"><span class="section-kicker">УПРАВЛЕНИЕ ДОСТУПОМ</span><h1>Безопасность аккаунта</h1><p>Контролируйте входы, активные сессии и блокировки пользователей.</p></div><button class="btn" onclick="nav('dashboard')">Вернуться на дашборд</button></div><div class="security-kpis"><div class="card"><span>Активные сессии</span><strong>${active.length}</strong><small>сейчас подключены</small></div><div class="card"><span>События входа</span><strong>${s.events.length}</strong><small>последние действия</small></div><div class="card"><span>Заблокированные</span><strong>${s.blocked.length}</strong><small>идентификаторов</small></div></div><div class="security-grid"><section class="card security-panel"><div class="security-panel-head"><div><span class="section-kicker">АКТИВНЫЕ СЕССИИ</span><h2>Кто сейчас в аккаунте</h2></div></div><div class="security-panel-note">Последние сессии · ${s.sessions.length} всего</div><div class="security-session-list">${previewSessions.map(item => `<article class="security-session ${item.status !== 'active' ? 'is-muted' : ''}"><div class="security-session-icon">${sessionIcon(item.method)}</div><div class="security-session-main"><div><strong>${esc(item.user)}</strong></div><p>${esc(methodLabel(item.method))} · ${esc(item.device)}</p><small>${esc(locationLabel(item))} · Последняя активность: ${esc(item.lastSeen)}</small></div><div class="security-session-actions">${item.status === 'active' && !item.current ? `<button class="icon-btn security-action-btn" aria-label="Завершить сессию" title="Завершить сессию" onclick="window.securityTerminate('${esc(item.id)}')">${actionIcon('terminate')}</button>` : `<span class="security-status ${esc(item.status)}">${statusLabel(item.status)}</span>`}${!item.current && item.status === 'active' ? `<button class="icon-btn security-block-btn" aria-label="Заблокировать пользователя" title="Заблокировать пользователя" onclick="window.securityBlock('${esc(item.identity)}')">⊘</button>` : ''}</div></article>`).join('')}</div><div class="security-panel-footer"><button class="btn small" onclick="window.openSecurityLogs('sessions')">Все логи</button></div></section><section class="card security-panel"><div class="security-panel-head"><div><span class="section-kicker">ЖУРНАЛ БЕЗОПАСНОСТИ</span><h2>История действий</h2></div></div><div class="security-panel-note">Последние события · ${s.events.length} всего</div><div class="security-event-list" id="securityEvents">${renderEvents(previewEvents, 'all')}</div><div class="security-panel-footer"><button class="btn small" onclick="window.openSecurityLogs('events')">Все события</button></div></section></div><section class="card security-panel security-blocked-panel"><div class="security-panel-head"><div><span class="section-kicker">БЛОКИРОВКИ</span><h2>Заблокированные пользователи</h2></div></div>${s.blocked.length ? s.blocked.map(identity => { const blocked = blockedDetails(s, identity); return `<div class="security-blocked-row"><div class="security-blocked-user"><strong>${esc(blocked.user)}</strong><small>${esc(identity)}</small></div><div class="security-blocked-date"><span>Последний визит</span><b>${esc(blocked.lastVisit)}</b></div><div class="security-blocked-date"><span>Заблокирован</span><b>${esc(blocked.blockedAt)}</b></div><button class="btn small" onclick="window.securityUnblock('${esc(identity)}')">Разблокировать</button></div>`; }).join('') : '<div class="security-empty">Заблокированных пользователей пока нет.</div>'}</section></div>`;
