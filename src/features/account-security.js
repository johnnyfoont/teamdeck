(function () {
  ...
  function locationRegion(item) { if (item.region) return item.region; const value = String(item.location || item.meta || '').toLowerCase(); if (/россия|беларус|казахстан|армени|азербайдж|кыргыз|молдова|таджики|туркмен|узбеки|украин/.test(value)) return 'Россия и СНГ'; if (/германи|франци|великобрит|итал|испан|польш|европ/.test(value)) return 'Европа'; if (/китай|япон|инд|коре|ази|турци|израил/.test(value)) return 'Азия'; if (/сша|америк|канад|мексик|бразил/.test(value)) return 'Америка'; if (/австрал|новая зеланд|океани/.test(value)) return 'Океания'; return 'Россия и СНГ'; }
  function locationLabel(item) { const value = item.country || item.location || 'Россия'; const flag = item.countryFlag || (/россия/i.test(value) ? '🇷🇺' : '🌐'); return `${flag} ${value}`.trim(); }
