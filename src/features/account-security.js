(function () {
  ...
  function formatTime(value) { const date = new Date(value); return Number.isNaN(date.getTime()) ? value : date.toLocaleString('ru-RU', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }); }
  async function loadRemoteEvents() {
    ...
    try {
      ...
      current.sessions = [...events.filter(event => event.type === 'login' && event.status === 'active').map(event => ({ id: `remote-${event.id}`, user: event.user, identity: event.identity || event.user, method: event.method, device: event.device, location: event.location, lastSeen: formatTime(event.lastSeen || event.time), status: 'active' })), ...current.sessions.filter(session => !session.id.startsWith('remote-'))];
      save(current);
      render();
    } catch (_) {}
  }
