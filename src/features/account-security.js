(function () {
  ...
  function filteredSessions(s) {
    ...
    const items = s.sessions.filter(item => !item.current && !query || [item.user, item.identity, item.method, item.device, item.location].some(value => String(value || '').toLowerCase().includes(query)))
  ...
  function render() {
    ...
    const s = state(); const active = s.sessions.filter(x => x.status === 'active' && !x.current); const previewSessions = s.sessions.filter(x => !x.current).slice(0, 12); const previewEvents = s.events.slice(0, 12);
