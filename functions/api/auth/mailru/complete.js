...
export async function onRequestGet({ request, env }) {
  ...
  const key = `oauth:mailru:session:${handoff}`;
  const profile = await env.TEAMDECK_KV.get(key, 'json');
  if (!profile?.email) return wantsJson ? json({ authenticated: false, error: 'handoff_expired' }, 404) : redirect('https://login.teamdeck.space/?error=mailru_handoff_expired');
